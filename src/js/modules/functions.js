export function global() {
  ...
  var f_windowWidth = function (w) {
    ...
    if (w >= n && !maxResize) {
      ...
      let headerListItem = document.querySelectorAll('.js-header__list li.submenu__item');
      let headerListLink = document.querySelectorAll('.js-header__list a.submenu__link');
      let listItemMenu = document.querySelector('.js-listitem');
      let adaptiveMenu = document.querySelector('.js-header__list');

      for (let i = 0; i < headerListItem.length; i++) {
        headerListItem[i].classList.remove('submenu__item');
        headerListItem[i].classList.add('header__list-item');
      }

      for (let i = 0; i < headerListLink.length; i++) {
        headerListLink[i].classList.remove('submenu__link');
        headerListLink[i].classList.add('header__list-link');
      }

      adaptiveMenu.classList.remove('submenu');
      adaptiveMenu.classList.add('header__list');


      let headerLogo = document.querySelector('.header__logo');

      headerLogo.insertAdjacentElement('afterend', adaptiveMenu)
